...
## Current state (as of 2026-06-08 / v45 selective oracle)

**13 validated deploy slots** clearing the standard biomechanics validity bar (Lin's CCC > 0.60 AND Bland-Altman 95% LoA half-width < ±10°) — **+1 vs v42/v40**. **14 slots at Tier 1 (CCC ≥ 0.79)** — unchanged. v45 integrates VideoPose3D 2D→3D pose lifting (Pavllo et al. 2019, FAIR, Apache 2.0) as a new Layer 2 reader (Agent TT, "v44" candidate). VideoPose3D is picked in **2 of 23 slots — both front-camera hip_adduction views**, exactly where 2D image geometry confounds adduction with flexion: `hip_adduction_r / front_oblique_right` **Moderate → Good** (CCC 0.889 → 0.960, LoA 13.8° → 8.5°) and `hip_adduction_r / front_center` Poor → Moderate (CCC 0.79 → 0.90). It regresses on the other 21 slots (side views, knee, lumbar — Human3.6M cameras are front-oriented, lateral motion lifts noisily); per-slot fallback preserves v42's pick everywhere v44 underperforms. **The mirror-twin bottleneck is upstream of geometry:** `hip_adduction_r / side_right` stayed Poor (v44 CCC 0.184 vs v43 0.277) — the cause is DWPose right-side depth bias, not 2D→3D ambiguity. VideoPose3D inference: 19 ms/clip mean (CPU), production-feasible. See `data/v45_selective_oracle/REPORT.md`.
