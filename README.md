...
## Current state (as of 2026-06-09 / v47 selective oracle)

**14 validated deploy slots** clearing the standard biomechanics validity bar (Lin's CCC > 0.60 AND Bland-Altman 95% LoA half-width < ±10°) — **+1 vs v45**. **14 slots at Tier 1 (CCC ≥ 0.79)** — unchanged. v47 is the first build to address **Layer 1** (the DWPose keypoint detector): test-time horizontal-flip augmentation (Agent UU, "v46" candidate). For each frame, run DWPose on the original AND its horizontal mirror, un-flip (mirror x + swap L/R keypoint labels), and confidence-weighted average — cancelling DWPose's left/right detector asymmetry. **Still single-camera** (two detector passes on the SAME frame, not multi-camera fusion). flip-TTA is the deploy reader for **2 of 23 slots**, and adds the new Good slot:
