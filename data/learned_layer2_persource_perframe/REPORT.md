# Per-Source Heads Learned Layer 2 — MM-A (per-source heads + per-frame SmoothL1) (Agent MM, Phase A)

**Date:** 2026-06-02
**Build:** Agent MM Phase A — mm_a variant
...
## Training recipe (mm_a)

  * **Loss**: masked per-frame SmoothL1 summed across the two heads (HH2-style loss).
  * **Optimizer**: AdamW lr=1e-3, weight_decay=1e-4, cosine LR.
  * **Step**: batch_size=256 frames (HH2-style frame batches), train_stride=2.
  * **Epochs**: 25.
  * **CPU only.**
...
## All-data L2 checkpoint (used by v26 Phase B)

  * **Training**: `mm_a_persource_*_alldata_v1`
  * **Checkpoint**: `models/learned_layer2_persource_perframe_alldata_v1.pt`
  * **LOSO discipline**: `ALL_DATA_NO_LOSO_AT_L2` — no LOSO at L2; this is the cached L2 used by v26 Phase B L3 ridge re-fit.

## Phase A 24-fold LOSO eval

**Not run this build cycle.** Same compute trade-off as the LL build: prioritized the all-data L2 → Phase B → v28 oracle path to answer Cameron's floor-lift question within the time budget. The harness supports Phase A LOSO via `python3 -m harness.learned_layer2_combined_persource --variant mm_a`.

## What mm_a actually delivered (Phase B preview)

See `data/layer3_retrain_persource_perframe/REPORT.md` for the full v26 Phase B tier table. The MM-brief headline question (did the floor lift on the 2 target supplementary slots?) is answered in `data/v28_selective_oracle/REPORT.md`.
...
4. **No invented numbers.** All metrics shown in the v28 report were computed on the cached all-data MM L2 checkpoint driving the v26 Phase B Layer 3 ridge re-fit.

## Files

  * `harness/learned_layer2_combined_persource.py` — MM trainer (architecture + MM-A + MM-B variants)
  * `harness/layer3_retrain_on_persource_perframe_l2.py` — mm_a Phase B driver
  * `models/learned_layer2_persource_perframe_alldata_v1.pt` — all-data mm_a L2 checkpoint
  * `data/layer3_retrain_persource_perframe/per_slot_validity_v26.json` — per-slot v26 validity stats (LOSO at L3 only)
